...
| FR-4.14 | The system ingests USGS NAS (Nonindigenous Aquatic Species) occurrence records per HUC8 via the federal v2 API at `nas.er.usgs.gov/api/v2/occurrence/search?huc8=X`. Filter to `group='Fishes'` and `status in {'established','stocked','collected','regularly observed'}`. Aggregate per (huc8, scientific_name) into `nas_occurrences` (huc8, scientific_name, common_name, status, year_first_observed, year_last_observed, n_records, source, ingested_at). | MUST | Integration test against the live API; row count > 0 for v0 launch states with documented introductions | M12 (2026-06-20); closes ML-01 differently than originally proposed |
| FR-4.15 | The system ingests USFS NorWeST mean-August stream-temperature climatology per processing unit via user-supplied shapefile under `${data_dir}/raw/norwest/<PU>/NorWeST_PredictedStreams_<PU>.shp`. Joins `COMID` to `xwalk_v2_to_hr` to produce HR COMIDs. Writes to `reach_temp_baseline` (comid, month, baseline_temp_c, source='NorWeST', ingested_at). Degrades cleanly when the shapefile is absent (logs warning, returns 0 rows). | MUST | Manual test: drop the Colorado PU shapefile under the conventional path, run `angler-ai ingest --source norwest --state CO`, verify > 5000 reach_temp_baseline rows | M12 (2026-06-20); FR-4.5 partial closure |
| FR-4.16 | The system honestly defers Colorado Parks and Wildlife stocking data ingestion to v1. The `ingest/cpw_stocking.py` stub raises `NotImplementedError` with v1 paths documented (FOIA of CPW master stocking table, scrape Fishery Survey Summary PDFs, or data-share agreement). Public CPW weekly report has only water+region+date — no species, no count — and `data.colorado.gov` has no CPW dataset (verified 2026-06-20). | MUST | `angler-ai ingest --source cpw_stocking` surfaces the deferral message, not a silent zero-row success | M12 (2026-06-20); CR-4 honesty pillar |
...
| FR-5.8 | Predictions explicitly tag the reach geography source (NHDPlus HR COMID) and the temperature source ('NWIS_obs' / 'NWIS_interp' / 'NWIS_interp_air_adjusted' / 'NWIS_air_projected' / 'NWIS_interp_air_adjusted_norwest_anchor' / 'NWIS_air_projected_norwest_anchor' / 'EcoSHEDS_TEMP' / 'PG-GNN' / 'not_modeled'). | MUST | Schema test on output | Pass 2 (NorWeST caveat); M12 NorWeST anchor tags |
| FR-5.9 | The system produces a per-reach per-day relative suitability index in [0, 1] over a 14-17 day forecast window by multiplying calibrated prior (BRT for native species, NAS-presence-only fallback for non-natives per FR-5.10) x species thermal niche x flow factor x seasonal phenology x flow z-score anomaly. Output is renamed `suitability_index` not `catch_probability` until M4 catch-data validation (multi-AI review STAT-02 / PV-6). | MUST | Schema test on `DailyScore` output | M7/M8 forecast pipeline; M12 NAS fallback |
| FR-5.10 | When `brt_priors` has no row for a (species, HUC8) pair AND `nas_occurrences` records the species as present in that HUC8, the system produces a wide-interval honest fallback prior with `point=0.35, lower=0.05, upper=0.65, interval_kind='spatial_unmodeled'`. The semantic is "presence confirmed at HUC8 scale, suitability unmodeled at reach scale" — the interval width is the honesty signal. Sources tuple includes `USGS_NAS_V1.0` and the NAS record metadata (status, year_last, n_records). Implemented as a Python-level router in `species_priors.species_priors_for_geometry` — NOT a SQL UNION (the two interval semantics must never blend). | MUST | Integration test: rainbow trout (O. mykiss) in HUC8 17040203 returns a SpeciesPrior with `interval_kind='spatial_unmodeled'` and half-width >= 0.20 | M12 (2026-06-20); closes ML-01 honestly |
| FR-5.11 | The `FactorBreakdown.base_source` field is derived from `cp.basis.sources[0]` with branch-aware labelling — preserves the honest hyperstability annotation regardless of underlying source. A BRT-derived prior surfaces `USGS_BRT_V2.0 (presence probability; hyperstability not_applied at v0)`; a NAS-derived prior surfaces `USGS_NAS_V1.0 (presence-only fallback; interval_kind=spatial_unmodeled)`. | MUST | Unit test on score_reach_daily with both prior types | M12 (2026-06-20) |
| FR-5.12 | When `reach_temp_baseline` has a NorWeST row for a (comid, month=8) tuple, the `water_temp_model.project_daily_temps` function uses it as the `T_water_baseline` for the Mohseni-Stefan projection in place of the stratified default. NorWeST is climatological — it is NOT in the `temperature.py::_PRIORITY` resolver chain and never surfaces as a current-day temperature source. | MUST | Unit test: with and without a NorWeST baseline, project_daily_temps returns different source tags (`NWIS_air_projected` vs `NWIS_air_projected_norwest_anchor`) | M12 (2026-06-20); FR-4.5 anchor role |
...
| FR-6.7 | `ProbabilityBasis` carries an `interval_kind` field with validated value in `{'sampling', 'spatial_unmodeled', 'climatological'}` distinguishing the semantic of the lower/upper interval: 'sampling' for survey-driven BRT-style intervals (typical half-width ≤ 0.10); 'spatial_unmodeled' for presence-only NAS-style intervals (half-width ≥ 0.20 by construction). Downstream surfaces (caption, narrative, JSON) MUST surface this so a brown-trout (sampling) and rainbow-trout (spatial_unmodeled) side-by-side in the same HUC8 reads correctly. The two interval semantics must never blend in a single output tuple (router-level enforcement in `species_priors_for_geometry`). | MUST | Unit test on `__post_init__` validation; integration test that a NAS-fallback prior has `interval_kind='spatial_unmodeled'` and its half-width >= 0.20 | M12 (2026-06-20); FR-6.4 extension |
...
| FR-9.4 | The batch driver (`.venv/run_{western,colorado}_forecasts.py`) produces a per-(water, species) directory structure: `summary.png` (best-day-over-window aggregate) + `daily/<YYYY-MM-DD>.png` (one PNG per day in the 14-17 day forecast window) + `daily_delta/<YYYY-MM-DD>.png` (one delta map per day-pair) + `animation.gif` (day-over-day animation). Each absolute-suitability map is matplotlib + RdYlGn colormap on [0, 1] with the colorbar label "Relative suitability index (0-1, NOT a probability)". Each delta map is matplotlib + RdBu diverging colormap centered at 0 with the colorbar label "Day-over-day change in suitability index (red = getting worse, blue = getting better; gray when |delta| < 0.02)". | MUST | Filesystem test + manual visual review | Per-day output milestone; M13 delta/animation additions |
...
| FR-9.8 | All rendered PNGs overlay reach geometry on a USGS National Map USTopo basemap (federal public-domain via `basemap.nationalmap.gov` through contextily). Reach lines render with a white halo (matplotlib `path_effects.Stroke`) for legibility against varied terrain. When contextily is missing, the provider fails to resolve, or the tile fetch fails, the renderer degrades to a plain white background AND surfaces an explicit caption note (`Basemap: not_modeled (...)`) — never silent. Tiles are cached locally by contextily so subsequent renders of the same HUC8 are offline. | MUST | Manual visual review; unit test of the three fallback paths | M13 (2026-06-20) |
| FR-9.9 | Batch driver runs land at `output/forecasts/<YYYY-MM-DD_HHMMSS>_<run-type>/<water>/<species>/...` where `<run-type>` distinguishes `colorado`, `western`, or future targets. The timestamped folder is created by the driver at startup via `_make_run_dir()`; no driver run overwrites another. | MUST | Filesystem test; two consecutive runs of the same driver produce two distinct timestamped folders | M14 (2026-06-20) |
...
| CR-4.7 | When a data source's public-facing surface lacks the fields needed to produce a useful prior (e.g., CPW weekly stocking report exposes only water+region+date with no species or count, verified 2026-06-20), the corresponding ingest module raises `NotImplementedError` with documented v1 paths rather than fabricating a value or shipping a sparse half-feature. The dispatcher MUST surface the deferral as `ok=False` in the IngestSummary; the explicit-only dispatch list keeps such modules out of the `all` order so a routine ingest run produces no noise. (M12 honest deferral pattern, lessons.md lesson 4.) | MUST |
| CR-4.8 | When a calibrated prior is derived from a fundamentally different uncertainty source (e.g., USGS NAS presence-only flag vs USGS BRT survey-driven probability), the `interval_kind` field on `ProbabilityBasis` records the semantic distinction. Downstream surfaces — caption, narrative, JSON, map renderer — MUST surface this so a viewer comparing two species side-by-side reads "wide NAS interval = we know they're here, we can't say where they're best" differently from "wide BRT interval = the survey was small". The two interval semantics MUST NOT blend in a single output tuple. (M12 FR-6.7 hard-couples to FR-5.10 router.) | MUST |
